...
import { ROOM_LIMITS, RoomType, SeatType } from "../constants";
import { Seat, SeatMap } from "../schemas/seat.schema";
...
export class RoomService {
  ...
  private buildSeatMap(
    seatMapRaw: (SeatType | null)[][]
  ): SeatMap {
    if (!Array.isArray(seatMapRaw) || seatMapRaw.length === 0)
      throw new BadRequestException('Seat map must be a non-empty 2D array');
    if (seatMapRaw.length < ROOM_LIMITS.MIN_ROWS || seatMapRaw.length > ROOM_LIMITS.MAX_ROWS)
      throw new BadRequestException(`Room must have between ${ROOM_LIMITS.MIN_ROWS} and ${ROOM_LIMITS.MAX_ROWS} rows`);

    return seatMapRaw.map((row, rowIdx) => {
      if (!Array.isArray(row) || row.length === 0)
        throw new BadRequestException(`Row ${rowIdx} must be a non-empty array`);
      if (row.length < ROOM_LIMITS.MIN_SEATS_PER_ROW || row.length > ROOM_LIMITS.MAX_SEATS_PER_ROW)
        throw new BadRequestException(`Row ${rowIdx} must have between ${ROOM_LIMITS.MIN_SEATS_PER_ROW} and ${ROOM_LIMITS.MAX_SEATS_PER_ROW} seats`);
      ...
      for (let colIdx = 0; colIdx < row.length; colIdx++) {
        ...
        if (seatType === 'COUPLE') {
          if (row[colIdx + 1] !== 'COUPLE') {
            throw new BadRequestException(`COUPLE seat at row ${rowIdx}, col ${colIdx} must be paired`);
          }
          seatRow.push({
  ...
  public async updateRoomById(
    ...
    if (roomName && roomName !== existed.roomName) {
      ...
      if (duplicated) throw new ConflictException(`Room [${roomName}] already exists`);
    }
